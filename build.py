...
# This ensures 02:00 AM matches group onto the correct day's HTML file.
...
for i in range(7):
    ...
    # FILTER: Match is only added if its LOCAL date matches the page date
    day_matches = []
    ...
    for m in day_matches:
        ...
        for c in m.get('tv_channels', []):
            ...
            for ch in c['channels']:
                ...
                # Store match object for channel page listing
...
for ch_name, matches in channels_data.items():
    ...
    for item in matches:
        ...
        <a href="{DOMAIN}/match/{slugify(m['fixture'])}/{dt.strftime('%Y%m%d')}/" class="match-row flex items-center p-4 bg-white border-b">
            <div class="time-box" style="min-width: 90px; text-align: center;">
                <div class="text-[10px] text-slate-400 font-bold auto-date" data-unix="{m['kickoff']}">{dt.strftime('%d %b')}</div>
                <div class="font-bold text-blue-600 auto-time" data-unix="{m['kickoff']}">{dt.strftime('%H:%M')}</div>
            </div>
            <div class="px-4"><span class="text-sm font-semibold">{m['fixture']}</span></div>
        </a>'''
